...
// function send_email($admin,$uid,$mail,$title,$type,$pass='123456',$username='',$bcc_email='zhangxiang@incinta.cn'){
//     $email = new Email();
////    type  1:审核成功； 2：审核拒绝；3：注册成功；4修改密码
//     $html = html($username,$mail,$pass,$admin,$type);
//     $res = $email->to($mail)
//         ->cc($bcc_email)
//        ->subject($title)
//        ->message($html)
...
function send_email($username,$mail,$pass="123456",$admin,$bcc_email='zhangxiang@incinta.cn',$type,$uid){
    $email = new Email();
    $t_email = TemplateEmail::find($type);
    $begin = '<div style="width: 700px;padding: 30px 70px 70px 70px;margin: 0 auto;">';
    $end = '</div>';
    $title = $t_email['title'];
    $html = $t_email['content'];
//        $img = Config::get('site.url');
    $img = 'https://vip.dopusa.com/uploads/logo.png';
    $html_logo = ' <img style="height:50px;margin: 0 auto;display: block;padding: 50px 70px 0px 70px;" src="'.$img.'" alt="">';
    if ($html){
        $company_phone = config('company.phone');
        $company_email = config('company.email');
        $html = str_replace("{begin}", $begin, $html);
        $html = str_replace("{logo}", $html_logo, $html);
        $html = str_replace("{name}", $username, $html);
        $html = str_replace("{email}", $mail, $html);
        $html = str_replace("{password}", $pass, $html);
        $html = str_replace("{admin}", $admin, $html);
        $html = str_replace("{company_phone}", $company_phone, $html);
        $html = str_replace("{company_email}", $company_email, $html);
...
function owner(){
    $ids =  \app\admin\model\RuleAssign::column('admin_id');
    $where=[
        'admin_id'=>['in',$ids]
    ];
    $admin_id = \app\admin\model\NewLead::where($where)->order('createtime','desc')->value('admin_id');

    $model = \app\admin\model\RuleAssign::where('admin_id',$admin_id)->find();

    $weight = \app\admin\model\RuleAssign::where('admin_id',$admin_id)->value('weigh');

    $admin_id = \app\admin\model\RuleAssign::where('weigh','<',$weight)->value('admin_id');
    if (!$admin_id){
        $admin_id = \app\admin\model\RuleAssign::order('weigh','desc')->value('admin_id');
    }
    return $admin_id;
}
