...
class Auth
{
     ...
     * @return boolean
     */
    public function register($first_name,$last_name, $password, $email = '', $mobile = '', $extend = [])
    {
        // 检测用户名、昵称、邮箱、手机号是否存在
...
//        }
        ...
        $user_model = User::where($where)->find();
        if ($email && $user_model) {
            $this->setError('Email already exist');
            return false;
        }
        $ip = request()->ip();
        $time = time();
        $g = 'https://www.gravatar.com/avatar/';
        $g = $g.md5($email);
        $data = [
...
//            'nickname'  => preg_match("/^1[3-9]{1}\d{9}$/",$username) ? substr_replace($username,'****',3,4) : $username,
     ...
     * @return boolean
     */
    public function login($account, $password,$service)
    {
        $field = Validate::is($account, 'email') ? 'email' : (Validate::regex($account, '/^1\d{10}$/') ? 'mobile' : 'username');
//        $user = User::get([$field => $account]);
        $user =\app\admin\model\User::where('service',$service)->where('email',$account)->find();
        if (!$user) {
